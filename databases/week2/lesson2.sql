...
SELECT task.id, task.title, user.email,user.name
FROM task 
JOIN user_task ON user_task.task_id = task.id
LEFT JOIN user ON task.id = user.id
WHERE task.id IN (SELECT id FROM user where user.email LIKE '%@spotify.com%');
SELECT task.title as task, user.name,status.id
FROM user
JOIN user_task ON user_task.user_id = user.id
JOIN task ON user_task.task_id = task.id
JOIN status on status.id = task.status_id
WHERE user.name LIKE '%Donald Duck%' and status.id = 1; 
SELECT task.title as task, user.name
FROM user
JOIN user_task ON user_task.user_id = user.id
JOIN task ON user_task.task_id = task.id
WHERE user.name LIKE '%Maryrose Meadows%' and month(created) = 09; 
SELECT MONTHNAME(task.created) as Month,
